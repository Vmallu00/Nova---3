...
async function rightClickNpc(npc) {
  ...
  const now = Date.now();
  if (now - lastNpcInteraction < 1500) return false;
  ...
  // Use plain coordinates for lookAt so non-Vec3 server entity positions
  // cannot cause Mineflayer's internal `point.minus is not a function`.
  await bot.lookAt({ x, y: y + 1.2, z }, true);
  await sleep(250);

  // Mineflayer's activateEntity can internally expect a real Vec3 position.
  // Clone the entity position into a Vec3-like object only for interaction.
  // The safest path is the protocol use_entity packet with mouse=0.
  ...
  if (typeof bot.activateEntity === "function") {
    ...
    log("SkyBlock NPC right-click sent.", "npc");
    status.npcInteracted = true;
    return true;
  }
