...
async function interactEntity(entity) {
  ...
  if (!entity) {
    throw new Error("NPC entity was not found.");
  }
  ...
  /*
   * Use the REAL Mineflayer entity.
   *
   * Never call:
   * entity.position.offset(...)
   *
   * because some server/entity packets can expose
   * position objects that are not Vec3 instances.
   */
  ...
  console.log(
    `[MINECRAFT] Right-clicking entity at ${x.toFixed(3)}, ${y.toFixed(3)}, ${z.toFixed(3)}`
  );

  /*
   * Look slightly above the entity's feet.
   */
  const lookPosition = new Vec3(
    x,
    y + 1.0,
    z
  );
   ...
   */
  if (
    typeof bot.activateEntity === "function"
  ) {
    await bot.activateEntity(entity);

    console.log(
      "[MINECRAFT] NPC right-click sent."
    );
  ...
  if (
    ...
    try {
      ...
      console.log(
        "[MINECRAFT] NPC interaction packet sent."
      );
      ...
      throw new Error(
        `NPC interaction failed: ${err.message}`
      );
    }
  }

  throw new Error(
    "This Mineflayer version has no entity interaction method."
  );
}
