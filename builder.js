...
function normalizeBlockName(name) {
  ...
  /*
   * Sponge states can look like:
   * minecraft:stone[variant=granite]
   *
   * Mineflayer's blockByName lookup
   * works with the base block name.
   */
  value =
    value.split("[")[0];
...
function sortBuildQueue(blocks) {
  /*
   * Build bottom-up, then Z, then X.
   * This gives structures a more stable foundation.
   */
  return [...blocks].sort(
    (a, b) =>
      a.y - b.y ||
...
function findInventoryMaterial(
  blockName
) {
  if (!bot?.inventory) {
    return null;
  ...
  try {
    ...
    if (
      ...
      updateBuild({
        ...
        action: "failed",
        error:
          `Could not place ${entry.block} at ${entry.x},${entry.y},${entry.z}`
      });
    }
